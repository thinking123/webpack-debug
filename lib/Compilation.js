...
class Compilation {
	...
	handleModuleCreation(
		...
		this.factorizeModule(//分解模块 获取文件和loader文件
	...
	addModuleChain(context, dependency, callback) {
		...
		const moduleFactory = this.dependencyFactories.get(Dep); //获取处理依赖的factory
		if (!moduleFactory) {
			return callback(
				new WebpackError(
		...
		this.hooks.optimizeTree.callAsync(this.chunks, this.modules, err => {
			...
			this.hooks.optimizeChunkModules.callAsync(
				...
				err => {
					...
					this.createModuleHashes();//生成每个module 的hash
					this.hooks.afterModuleHash.call();
					this.logger.timeEnd("module hashing");

					this.logger.time("code generation");
					this.hooks.beforeCodeGeneration.call();
					this.codeGenerationResults = this.codeGeneration();//source 内容：包括导入和导出 -> string
					this.hooks.afterCodeGeneration.call();
					this.logger.timeEnd("code generation");

					this.logger.time("runtime requirements");
					this.hooks.beforeRuntimeRequirements.call();
					this.processRuntimeRequirements(this.entrypoints.values());//设置runtime requiments
