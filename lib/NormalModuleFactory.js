...
class NormalModuleFactory extends ModuleFactory {
	constructor({ context, fs, resolverFactory, options }) {
		...
		this.hooks.factorize.tapAsync(
			...
			(resolveData, callback) => {
				this.hooks.resolve.callAsync(resolveData, (err, result) => {//获取文件和文件的loader：resolveData
					...
					this.hooks.afterResolve.callAsync(resolveData, (err, result) => {
						...
						// Ignored
						if (result === false) return callback();
						//createData ： loader ， generator ， parser 等
						...
						if (!createdModule) {
							...
							createdModule = new NormalModule(createData);//对应一个文件
						}
	...
	getParser(type, parserOptions = EMPTY_OBJECT) {//type -> cache -> parserOptions -> parser
